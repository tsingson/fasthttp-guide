...
func (ws *webServer) simpleGetHandler() func(ctx *fasthttp.RequestCtx) {
	return func(ctx *fasthttp.RequestCtx) {
		...
		tid := utils.B2S(ctx.Request.Header.Peek("TransactionID"))
		l := ws.Log.Named(tid)
		...
		if ws.debug {
			...
			ctx.Request.Header.VisitAll(func(key, value []byte) {
				...
				l.Debug(tid, zap.String("key", utils.B2S(key)), zap.String("value", utils.B2S(value)))
			})

			l.Debug(tid, zap.String("http payload", utils.B2S(ctx.Request.Body())))
