...
func Generate(alphabet string, size int) ([]byte , error) {
	if len(alphabet) == 0 || len(alphabet) > 255 {
		return nil , fmt.Errorf("alphabet must not empty and contain no more than 255 chars. Current len is %d", len(alphabet))
	}
	if size <= 0 {
		return nil , fmt.Errorf("size must be positive integer")
	}
